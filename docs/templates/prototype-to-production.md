...
flowchart TB
    ...
    subgraph Entry["Entry"]
        Client[Client] --> FastAPI[FastAPI] --> Pydantic[Pydantic]
    end

    %% Agent Core
    subgraph AgentCore["Agent Core"]
        HumanReview[Human review] <--> LangGraph[LangGraph]
        LangGraph --> LiteLLM[LiteLLM]
        LiteLLM -. Retry / Fallback .-> LiteLLM
    ...
    UVRuff -. packages .-> AgentCore

    %% Quality
    subgraph Quality["Quality"]
        Langfuse[Langfuse]
        Pytest[Pytest] --> Validation[Validation]
        LangGraph -. traces .-> Langfuse
        Pytest -. runs .-> Validation
    end

    %% Ship
    subgraph Ship["Ship"]
        GHA[GitHub Actions] --> Docker[Docker]
    end
    GHA -- builds --> Docker
    Docker -. image .-> Ship
    ...
    Docker -- deploys to --> AWS
    Docker -- deploys to --> GCP
```
